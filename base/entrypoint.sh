...
if [ -n "$GANGLIA_HOST" ]; then
    mv $HADOOP_HOME/etc/hadoop/hadoop-metrics.properties $HADOOP_HOME/etc/hadoop/hadoop-metrics.properties.orig
    mv $HADOOP_HOME/etc/hadoop/hadoop-metrics2.properties $HADOOP_HOME/etc/hadoop/hadoop-metrics2.properties.orig

    for module in mapred jvm rpc ugi; do
        echo "$module.class=org.apache.hadoop.metrics.ganglia.GangliaContext31"
        echo "$module.period=10"
        echo "$module.servers=$GANGLIA_HOST:8649"
    done > $HADOOP_HOME/etc/hadoop/hadoop-metrics.properties
    ...
    done > $HADOOP_HOME/etc/hadoop/hadoop-metrics2.properties
fi
